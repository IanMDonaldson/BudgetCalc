...
def create_xl_from_dates(begin_date, end_date):
    ...
    for classification, counter in classifications_and_count:
        transactions_by_classification = TransactionRepository.get_transactions_by_classification_and_date(
            classification, begin_date, end_date)
        ...
        if end_row != row:
            ...
            worksheet.write(end_row, 4,
                            '=SUM(E{start_row}:E{current_row})'.format(start_row=row + 1, current_row=end_row))
